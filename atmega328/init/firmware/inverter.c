...
ISR(TIMER0_COMPA_vect) {
	if((phase_idx[0] >> 8) & 1)
		PORTC |= 0b11; // abuse two's complement: if phase_idx MSB has even parity, make PORTC[7:2] | 0b11 (pull-up)
	else
		PORTC &= 0b111100;
...
ISR(TIMER0_COMPB_vect) {
	...
	if((phase_idx[1] >> 8) & 1)
		PORTC |= 0b1100; // abuse two's complement: if phase_idx MSB has even parity, make PORTC[7:2] | 0b11 (pull-up)
	else
		PORTC &= 0b110011;
	// TIFR0 |= 0b100;
}
